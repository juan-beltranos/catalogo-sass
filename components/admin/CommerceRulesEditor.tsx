    ...
    <section className="space-y-3 border-t pt-5">
      ...
      {value.shipping.map((rule) => <div key={rule.id} className="space-y-3 rounded-xl border p-4">
        ...
        <div className="grid grid-cols-2 gap-2 lg:grid-cols-7">
          ...
          <SelectField label="Metodo" value={rule.shippingMethod || "all"} onChange={(shippingMethod) => patchShipping(rule.id, { shippingMethod: shippingMethod as ShippingRule["shippingMethod"] })} options={[["all", "Todos"], ["pickup", "Recoger en tienda"], ["local", "Entrega local"], ["national", "Envio nacional"]]} />
