...
const dbStoreToApp = async (row: any) => {
  ...
  return {
    ...
    shippingMethods: (shipping.methods ?? ["pickup"]).map((method: string) => method === "cod" ? "pickup" : method === "carrier" ? "local" : method),
    shippingCostPickup: shipping.costPickup ?? shipping.costCOD ?? 0,
    shippingCostLocal: shipping.costLocal ?? shipping.costCarrier ?? 0,
    shippingCostCOD: shipping.costCOD ?? 0,
    shippingCostCarrier: shipping.costCarrier ?? 0,
    shippingCostNational: shipping.costNational ?? 0,
...
const appStoreToDb = (data: any) => {
  ...
  if (shippingKeys.some((key) => key in data)) {
    payload.shipping_settings = {
      ...
      methods: data.shippingMethods ?? ["pickup"],
      costPickup: Number(data.shippingCostPickup ?? data.shippingCostCOD ?? 0),
      costLocal: Number(data.shippingCostLocal ?? data.shippingCostCarrier ?? 0),
      costCOD: Number(data.shippingCostCOD ?? 0),
      costCarrier: Number(data.shippingCostCarrier ?? 0),
      costNational: Number(data.shippingCostNational ?? 0),
